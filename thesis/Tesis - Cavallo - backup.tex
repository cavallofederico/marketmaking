...
TODO: Agregar explicación matricial en pseudocódigo

\subsubsection{Consideraciones técnicas}

% Scipy minimize
Se utilizó una implementación del método de optimización Nelder-Mead de la librería \texttt{scipy} que se invoca utilizando la función \texttt{minimize}. El ejemplo proveído por la librería es el de la optimización de la función \texttt{rosen} está en la Figura \ref{code:scipyexample}. 
...
\end{lstlisting}
\caption{Función a minimizar.}
	\label{code:funcionaminimizar}
\end{figure}


Para resolver esto se debe expresar los datos en forma matricial, operarlos y finalmente realizar una sumatoria. En primer lugar se define la parte central de código en la Figura \ref{code:funcionaamaximizar}
\begin{figure}[H]
\begin{lstlisting}[style=mypython]
    def likelihood(self, x):
        ...
        return likelihood
\end{lstlisting}
\caption{Función a maximizar.}
	\label{code:funcionaamaximizar}
\end{figure}
El primer término es trivial, en el segundo término se intenta calcular la sumatoria sobre alfa. Para ello se plantean una serie de vectores y matrices.

Definimos
\begin{equation}
\tau_0 = [0, \boldsymbol{\tau_0^-}, \boldsymbol{\tau_0^+}, T] 
\end{equation}

los vectores $\boldsymbol{\eta}$

\begin{align*}
\boldsymbol{\eta^{-}} &= -\eta^- \cdot \mathbf{1} \\
\boldsymbol{\eta^+} &= \eta^+ \cdot \mathbf{1} \\
\boldsymbol{\eta} &= [0, \boldsymbol{\eta^-}, \boldsymbol{\eta^+}, 0]^T \\
\end{align*}
...
Luego se ordena \(\boldsymbol{\tau\eta}\) en base a los valores de \(\boldsymbol{\tau_0}\).

Para hacerlo más amigable, se puede plantear un ejemplo donde se tienen los vectores de tiempos:
